// ====== AMBIL MEDIA DARI CONFIG (AUTO) ======
document.addEventListener("DOMContentLoaded", () => {
  try {
    const bgm = document.getElementById("bgm");
    if (bgm && window.APP_CONFIG?.audio) bgm.src = APP_CONFIG.audio;
    const photo = document.getElementById("profilePhoto");
    if (photo && window.APP_CONFIG?.photo) photo.src = APP_CONFIG.photo;
    const gift = document.getElementById("giftImg");
    if (gift && window.APP_CONFIG?.gift) gift.src = APP_CONFIG.gift;
    const og = document.querySelector('meta[property="og:image"]');
    const tw = document.querySelector('meta[name="twitter:image"]');
    if (og) og.setAttribute("content", APP_CONFIG.preview || APP_CONFIG.photo);
    if (tw) tw.setAttribute("content", APP_CONFIG.preview || APP_CONFIG.photo);
  } catch(e) {}
});
